"""
Separa Object_14_Compuerta en 4:
  Object_14_PanelL   -> tz >= 6.5, tx >= 0
  Object_14_PanelR   -> tz >= 6.5, tx < 0
  Object_14_BaseCano -> tz < 6.5, ty > -0.41
  Object_14_BaseDoor -> tz < 6.5, ty <= -0.41
"""
import bpy, bmesh
...
src = bpy.data.objects.get("Object_14_Compuerta")
if not src:
    print("[!] Object_14_Compuerta no encontrado"); raise SystemExit

M = src.matrix_world
print(f"Object_14_Compuerta: {len(src.data.vertices)} verts")
...
for start in bm.verts:
    ...
    tz  = -cy
    if tz >= 6.5:
        zone = "NoseDoorFwdL" if cx >= 0 else "NoseDoorFwdR"
    elif ty > -0.41:
        zone = "BaseCano"
    else:
        zone = "NoseDoorAft"
...
for z in ["NoseDoorFwdL", "NoseDoorFwdR", "BaseCano", "NoseDoorAft"]:
