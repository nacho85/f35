...
OUT_DIR  = r"C:\devs\f35\scripts\f14_splits_verify"
SUSPECTS = ["Object_19_R","Object_19_L","Object_19_C",
            "Object_20_R","Object_20_L","Object_20_C"]
...
def cam(objs):
    ...
    half = max(max(abs(v.x-cx) for v in cs), max(abs(v.y-cy) for v in cs), 3) * 1.5
    ...
    co.location=(cx,cy,cz+30); co.rotation_euler=(0,0,0)
