"""Analyze loose pieces of Object_20_tailwing_R/L by X and Y centroid."""
...
for base_name in ["Object_20_tailwing_R", "Object_20_tailwing_L"]:
    ...
    print(f"\n{base_name}: {len(pieces)} pieces")
    for p in sorted(pieces, key=lambda o: abs(centroid(o)[0])):
        cx, cy = centroid(p)
        print(f"  cx={cx:>7.2f}  cy={cy:>6.2f}  verts={len(p.data.vertices):>4}  {p.name}")
