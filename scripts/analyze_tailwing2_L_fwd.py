...
base = bpy.data.objects.get("Object_20_tailwing2_L_fwd")
if base is None:
    ...
    print(f"\nObject_20_tailwing2_L_fwd: {len(pieces)} piezas")
    print(f"{'cx':>8}  {'cy':>8}  {'verts':>6}")
    for p in sorted(pieces, key=lambda o: centroid(o)[1]):
        cx, cy = centroid(p)
        print(f"{cx:>8.3f}  {cy:>8.3f}  {len(p.data.vertices):>6}")
