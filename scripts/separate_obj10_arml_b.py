"""
Separa Object_10_ArmL_B en 2 por centroide de cluster:
  Object_10_ArmL_B  → tz < 6.70  (pieza aft/inferior)
  Object_10_ArmL_C  → tz >= 6.70 (pieza fwd/superior izquierda)
"""
import bpy, bmesh
...
orig = next((o for o in bpy.data.objects if o.type == "MESH" and o.name == "Object_10_ArmL_B"), None)
if not orig:
    print("[!] Object_10_ArmL_B no encontrado"); raise SystemExit

M = orig.matrix_world
orig.name      = "_ArmLB_src_"
orig.data.name = "_ArmLB_src_mesh_"
print(f"Object_10_ArmL_B: {len(orig.data.vertices)} verts")
...
for start in bm.verts:
    ...
    tz  = -cy
    zone = "C" if tz >= 6.70 else "B"
    for vi in comp:
        zone_for_vert[vi] = zone

bm.free()

ZONES = {"B": "ArmL_B", "C": "ArmL_C"}
