"""
Separa Object_16001 en 2 por centroide de cluster:
  Object_16001_NoseGear  → tz >= 5.0  (tren delantero)
  Object_16001_RearGearL → tz <  5.0  (tren trasero izquierdo)

Lee/escribe F-14-iran-v4.glb
"""
import bpy, bmesh
...
orig = next((o for o in bpy.data.objects if o.type == "MESH" and o.name == "Object_16.001"), None)
if not orig:
    print("[!] Object_16.001 no encontrado"); raise SystemExit

M = orig.matrix_world
print(f"Object_16001: {len(orig.data.vertices)} verts")
...
for start in bm.verts:
    ...
    tz  = -cy  # Three.js z (nariz = positivo)
    zone = "NoseGear" if tz >= 5.0 else "RearGearL"
    for vi in comp:
        zone_for_vert[vi] = zone

bm.free()

ZONES = {"NoseGear": "NoseGear", "RearGearL": "RearGearL"}
for z in ZONES:
...
for z, label in ZONES.items():
    ...
    new_obj.name      = f"Object_16001_{label}"
    new_obj.data.name = f"Object_16001_{label}_mesh"
