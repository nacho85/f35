...
function createRearWheelHeuristicOverlay(target, tone, scoreMode = 'heuristic', rimTone = null) {
  const sourceGeometry = target?.geometry;
  const positionAttr = sourceGeometry?.getAttribute("position");
  ...
  if (scoreMode === 'allCircular') {
    ...
    const allDiscComps = sizedComponents
      .filter((c) => {
        ...
        return d1 / (d2 || 0.001) > 0.60 && d0 / (d2 || 0.001) < 0.68 && d2 > 0.5 && d1 > 0.4;
      })
    ...
    // Split by volume: large discs = rubber tread (tone/black);
    // small discs = inner rim rings / hub caps (rimTone/silver).
    const maxVol = allDiscComps[0].size.x * allDiscComps[0].size.y * allDiscComps[0].size.z;
    const treadComps   = allDiscComps.filter((c) => c.size.x * c.size.y * c.size.z >= maxVol * 0.30);
    const discRimComps = allDiscComps.filter((c) => c.size.x * c.size.y * c.size.z <  maxVol * 0.30);
